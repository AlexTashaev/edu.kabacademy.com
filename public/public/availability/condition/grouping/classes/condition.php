...
class condition extends \core_availability\condition {
    ...
    public function filter_user_list(array $users, $not, \core_availability\info $info,
        ...
        global $CFG, $DB;
        // KAB perf fix: request-level cache for grouping members. Without it, calendar/completion
        // code paths re-run this SQL for every cm with the same grouping (N+1 on big courses).
        static $groupingusersmemo = [];
        ...
        $groupingid = $this->get_grouping_id($info);
        if (!isset($groupingusersmemo[$groupingid])) {
            $groupingusersmemo[$groupingid] = $DB->get_records_sql("
        ...
        $groupingusers = $groupingusersmemo[$groupingid];
