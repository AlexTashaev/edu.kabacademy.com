...
class condition extends \core_availability\condition {
    ...
    public function filter_user_list(array $users, $not, \core_availability\info $info,
        ...
        global $CFG, $DB;
        // KAB perf fix: request-level cache for group members. Calendar/completion code
        // re-enters this for every cm with the same group → N+1 on big courses.
        static $groupusersmemo = [];
        ...
        $cachekey = $this->groupid ? 'g:' . $this->groupid : 'c:' . $course->id;
        if (!isset($groupusersmemo[$cachekey])) {
            if ($this->groupid) {
                $groupusersmemo[$cachekey] = groups_get_members($this->groupid, 'u.id', 'u.id ASC');
            } else {
                $groupusersmemo[$cachekey] = $DB->get_records_sql("
        ...
        $groupusers = $groupusersmemo[$cachekey];
