...
class capability_checker {
    ...
    public function get_users_by_capability($capability) {
        // KAB perf fix: request-level cache shared across all capability_checker instances.
        // Original $this->cache is per-instance and gets recreated for every cm/condition,
        // causing N+1 calls to get_users_by_capability() on calendar/completion code paths.
        static $requestcache = [];
        $contextid = $this->context->id;
        if (!isset($requestcache[$contextid][$capability])) {
            $requestcache[$contextid][$capability] = get_users_by_capability(
                    $this->context, $capability, 'u.id');
        }
        if (!array_key_exists($capability, $this->cache)) {
            $this->cache[$capability] = $requestcache[$contextid][$capability];
        }
